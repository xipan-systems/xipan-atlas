...
impl LandingGearController {
    ...
    fn update(&mut self, command: PilotCommand, sensors: Sensors) -> ActuatorCommand {
        match self.state {
            GearState::DownLocked => {
                if matches!(command, PilotCommand::GearUp) {
                    // Weight-on-wheels interlock: never retract on the ground.
            ...
            GearState::Retracting => {
                if all_up_locked(&sensors) {
                    self.state = GearState::UpLocked;
                    ActuatorCommand::Hold
                } else {
                    ActuatorCommand::Retract
                }
